...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      // PKCE flow: exchange the code from URL query param for a session
      if (code) {
      ...
      // Implicit flow fallback: check for existing session (hash-based)
      ...
      // Listen for PASSWORD_RECOVERY event (hash flow)
      ...
      // Give time for the hash to be processed
      setTimeout(async () => {
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[2.5rem] shadow-2xl shadow-blue-100/50 p-8 md:p-10">
          ...
          {!success && sessionReady && !sessionError && (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              >
                {loading
                  ? <><Loader2 className="w-5 h-5 animate-spin mr-2" /> Updating password...</>
                  : 'Update Password'
                }
              </button>
            </form>
